...
## Rapid Sketching (Variant Exploration)

Use when the user wants to **see a design direction before committing** — "sketch this screen", "show me what X could look like", "compare layout A vs B", "2-3 takes on this UI". Disposable HTML mockups, not production code.

**Core method:** `intake → variants → head-to-head → pick winner (or iterate)`

1. **Intake** — get three things (one question at a time): feel/adjectives, references, and the core user action. Skip if user already gave enough.
2. **Variants (2-3, never 1)** — each takes a **different design stance**, not different pixel values. Variant axes: density, emphasis (content-first / action-first / tool-first), aesthetic (editorial / utilitarian / playful), layout (single-column / sidebar / split-pane), grounding (card-based / bare-content / document-style). Pick one axis and pull apart.
3. **Make them real HTML** — single self-contained file per variant, inline CSS, real content (not lorem ipsum), interactive (at least one state transition). Verify visually with browser tools before showing.
4. **Head-to-head** — present comparison table with opinionated take. Let user pick, combine, or iterate.
5. **Output** — `sketches/NNN-stance-name/index.html` + `README.md` per variant. Keep disposable.

**Interactivity bar:** user can click a primary action (state change), see one meaningful state transition, and hover recognizable affordances. More is over-engineering a throwaway.

**Frontier mode** (what to sketch next): consistency gaps between winning variants, unsketched screens, state coverage (empty/loading/error), responsive gaps, interaction patterns.

**If `gsd-sketch` is installed** (full GSD system), prefer it for persistent `.planning/sketches/` with MANIFEST and consistency audits.

## Creative Coding with Pretext (@chenglou/pretext)

[`@chenglou/pretext`](https://github.com/chenglou/pretext) — 15KB zero-dependency TS library for DOM-free multiline text measurement. Given `(text, font, width)`, returns line breaks, per-line widths, per-grapheme positions, and total height. It's a **creative primitive** for: text flowing around shapes at 60fps, games with text-as-geometry, kinetic typography, generative art with real prose.

**When to use:** "pretext demo", text flowing around moving shapes, ASCII-art with proportional fonts, text-as-bricks games, kinetic typography, generative text art, multiline shrink-wrap UI.

**Stack:** single HTML file, `@chenglou/pretext` via `esm.sh`, Canvas 2D, `Intl.Segmenter` for grapheme splitting.

**Two core shapes:**

| Shape | API | Use for |
|---|---|---|
| Measure only (CSS renders) | `prepare()` + `layout()` → height/lineCount | Virtualized lists, masonry, "does this fit?" |
| Measure + render yourself | `prepareWithSegments()` + `layoutWithLines()` → lines[] with positions | Canvas rendering, per-glyph transforms, creative work |

**Key pattern — variable-width flow** (text around obstacles):
```js
let cursor = { segmentIndex: 0, graphemeIndex: 0 };
let y = 0;
while (true) {
  const lineWidth = widthAtY(y);  // your geometry function
...
**Creative standards:** dark backgrounds, proportional fonts (never default sans), real prose (never lorem), first-paint excellence, at least one interactive response. Starter templates: `templates/hello-orb-flow.html` (orb reflow), `templates/donut-orbit.html` (advanced: ASCII masks, draggable shapes). Pattern reference: `references/pretext-patterns.md`.

**Performance:** `prepare()` is expensive — call once, cache. `layout*` is cheap per frame. Set `ctx.font` once per frame, not per `fillText`.

**Pretext pitfalls:** drifting CSS/canvas font strings (preload fonts), re-preparing inside animation loop, monospace fallbacks erasing the point, shipping a "cold" demo without vignette/auto-motion/interaction.
